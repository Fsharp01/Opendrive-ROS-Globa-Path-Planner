            ...
            if (road.lanes.laneSections[0].left):
                for lane in road.lanes.laneSections[0].left:
                    if (lane.type == "driving"):
                        ...
                        lane_width_offset = 0
                        for i in range(0, abs(lane.id)):
                            lane_width_offset += road.lanes.laneSections[0].left[-1 - i].width[
                                0].a  # -1-i because the list must be checked inside out
                        # total lane offset for lane origin
                        lane_offset = lane_width_offset - lane.width[0].a / 2 + road.lanes.laneOffset[0].a
                        # lane origin center
                        waypoint_lane_origin_center = T4ac_Waypoint()
                        waypoint_lane_origin_center.transform.location = self.get_point_in_line(road.planView[0].x,
                                                                                                road.planView[0].y, 0,
                                                                                                lane_offset,
                                                                                                road.planView[
                                                                                                    0].hdg + math.pi / 2)
                        waypoint_lane_origin_center.transform.location.z = self.calculate_elevation(
                            road.elevationProfile[0].a, road.elevationProfile[0].b, road.elevationProfile[0].c,
                            road.elevationProfile[0].d, road.elevationProfile[0].s, 0)
                        ...
                        if (road.planView[-1].type == "line"):
                            ...
                            aux_location = self.get_point_in_line(road.planView[-1].x, road.planView[-1].y, 0,
                                                                  lane_offset, road.planView[-1].hdg + math.pi / 2)
                            waypoint_lane_end_center.transform.location = self.get_point_in_line(aux_location.x,
                                                                                                 aux_location.y, 0,
                                                                                                 road.planView[
                                                                                                     -1].length,
                                                                                                 road.planView[-1].hdg)
                            waypoint_lane_end_center.transform.location.z = self.calculate_elevation(
                                road.elevationProfile[-1].a, road.elevationProfile[-1].b, road.elevationProfile[-1].c,
                                road.elevationProfile[-1].d, road.elevationProfile[-1].s, road.planView[-1].length)
                            ...
                            aux_location = self.get_point_in_line(road.planView[-1].x, road.planView[-1].y, 0,
                                                                  lane_offset, road.planView[-1].hdg + math.pi / 2)
                            waypoint_lane_end_center.transform.location = self.get_point_in_arc(aux_location.x,
                                                                                                aux_location.y, 0,
                                                                                                new_length,
                                                                                                new_curvature,
                                                                                                road.planView[-1].hdg)
            ...
            if (road.lanes.laneSections[-1].left):
                for lane in road.lanes.laneSections[-1].left:
                    if (lane.type == "driving"):
                        ...
                        lane_width_offset = 0
                        for i in range(0, abs(lane.id)):
                            lane_width_offset += road.lanes.laneSections[-1].left[-1 - i].width[
                                -1].a  # -1-i because the list must be checked inside out
                        # total lane offset for lane origin
                        ...
                        for geometry in road.planView:
                            ...
                            waypoint_geometry_origin_lane_center.transform.location = self.get_point_in_line(geometry.x,
                                                                                                             geometry.y,
                                                                                                             0,
                                                                                                             lane_offset,
                                                                                                             geometry.hdg + math.pi / 2)
                            waypoint_geometry_origin_lane_center.transform.location.z = self.calculate_elevationProfile(
                                road.elevationProfile, waypoint_geometry_origin_lane_center.s)
                            waypoint_geometry_origin_lane_center.transform.rotation.yaw = (geometry.hdg) * (
                                        180 / math.pi) + 90
                            ...
                            if (geometry.type == "line"):
                                ...
                                for n_dist in range(0, n):
                                    ...
                                    waypoint.transform.location = self.get_point_in_line(
                                        waypoint_geometry_origin_lane_center.transform.location.x,
                                        waypoint_geometry_origin_lane_center.transform.location.y, 0, n_dist * distance,
                                        geometry.hdg)
                                ...
                                waypoint.transform.location = self.get_point_in_line(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0, geometry.length,
                                    geometry.hdg)
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (geometry.hdg) * (180 / math.pi) + 90
                                waypoint.lane_width = lane.width[0].a
                                waypoint.road_id = road.id
                                ...
                                for n_dist in range(0, n):
                                    waypoint = T4ac_Waypoint()
                                    waypoint.s = geometry.s + n_dist * distance
                                    waypoint.transform.location = self.get_point_in_arc(
                                        waypoint_geometry_origin_lane_center.transform.location.x,
                                        waypoint_geometry_origin_lane_center.transform.location.y, 0, n_dist * distance,
                                        new_curvature, geometry.hdg)
                                    waypoint.transform.location.z = self.calculate_elevationProfile(
                                        road.elevationProfile, waypoint.s)
                                    waypoint.transform.rotation.yaw = (geometry.hdg + (
                                                n_dist * distance) * new_curvature) * (180 / math.pi) + 90
                                    waypoint.lane_width = lane.width[0].a
                                    waypoint.road_id = road.id
                                    waypoint.lane_id = lane.id
                                    waypoint.junction = road.junction
                                ...
                                waypoint.transform.location = self.get_point_in_arc(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0, new_length,
                                    new_curvature, geometry.hdg)
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (geometry.hdg + (new_length) * new_curvature) * (
                                            180 / math.pi) + 90
                                waypoint.lane_width = lane.width[0].a
                                waypoint.road_id = road.id
                                waypoint.lane_id = lane.id
            ...
            if (lane.type == "driving"):
                ...
                for geometry in reversed(road.planView):
                    ...
                    if (geometry.type == "line"):
                        ...
                        waypoint.transform.location = self.get_point_in_line(
                            waypoint_geometry_origin_lane_center.transform.location.x,
                            waypoint_geometry_origin_lane_center.transform.location.y, 0, geometry.length, geometry.hdg)
                        waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                        waypoint.s)
                        waypoint.transform.rotation.yaw = (geometry.hdg) * (180 / math.pi) + 90
                        waypoint.lane_width = lane.width[0].a
                        ...
                        waypoint.transform.location = self.get_point_in_arc(
                            waypoint_geometry_origin_lane_center.transform.location.x,
                            waypoint_geometry_origin_lane_center.transform.location.y, 0, new_length, new_curvature,
                            geometry.hdg)
                        waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                        waypoint.s)
                        waypoint.transform.rotation.yaw = (geometry.hdg) * (180 / math.pi) + 90
                        ...
                        for n_dist in range(n, -1, -1):
                            ...
                            waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                            waypoint.s)
                            waypoint.transform.rotation.yaw = (geometry.hdg + (n_dist * distance) * new_curvature) * (
                                        180 / math.pi) + 90
                            waypoint.lane_width = lane.width[0].a
                            waypoint.road_id = road.id
                            waypoint.lane_id = lane.id
            ...
            if (lane.type == "driving"):
                ...
                for i in range(geometry_index, -1, -1):
                    ...
                    if i == geometry_index:
                        # lane origin center
                        waypoint_geometry_origin_lane_center = T4ac_Waypoint()
                        waypoint_geometry_origin_lane_center.transform.location = self.get_point_in_line(
                            road.planView[i].x, road.planView[i].y, 0, lane_offset, road.planView[i].hdg + math.pi / 2)
                        # waypoint_geometry_origin_lane_center.transform.location.z = calculate_elevationProfile(road.elevationProfile, 0)
                        waypoint_geometry_origin_lane_center.road_id = road.id
                        waypoint_geometry_origin_lane_center.lane_id = lane.id
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            waypoint.transform.location = self.get_point_in_line(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s_0 - road.planView[i].s,
                                road.planView[i].hdg)
                            waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                            waypoint.s)
                            ...
                            waypoint.transform.location = self.get_point_in_arc(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s_0 - road.planView[i].s,
                                new_curvature, road.planView[i].hdg)
                            waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                            waypoint.s)
                            waypoint.transform.rotation.yaw = (road.planView[i].hdg) * (180 / math.pi) + 90
                            waypoint.lane_width = lane.width[0].a
                            ...
                            for n_dist in range(n, -1, -1):
                                ...
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (road.planView[i].hdg + (
                                            n_dist * distance) * new_curvature) * (180 / math.pi) + 90
                                waypoint.lane_width = lane.width[0].a
                                waypoint.road_id = road.id
                                waypoint.lane_id = lane.id
                    ...
                    # For rest of geometries get every wp
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            waypoint.transform.location = self.get_point_in_line(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, road.planView[i].length,
                                road.planView[i].hdg)
                            waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                            waypoint.s)
                            waypoint.transform.rotation.yaw = (road.planView[i].hdg) * (180 / math.pi) + 90
                    ...
                    if i == geometry_index:
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            for n_dist in range(0, n):
                                ...
                                waypoint.transform.location = self.get_point_in_line(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0,
                                    s_0 - waypoint_geometry_origin_lane_center.s + (n_dist * distance),
                                    road.planView[i].hdg)
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (road.planView[i].hdg) * (180 / math.pi) + 90
                            ...
                            for n_dist in range(0, n):
                                ...
                                waypoint.transform.location = self.get_point_in_arc(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0,
                                    s_0 - waypoint_geometry_origin_lane_center.s + (n_dist * distance), new_curvature,
                                    road.planView[i].hdg)
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (road.planView[i].hdg + (
                            ...
                            waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                            waypoint.s)
                            waypoint.transform.rotation.yaw = (road.planView[i].hdg + (new_length) * new_curvature) * (
                                        180 / math.pi) + 90
                            waypoint.lane_width = lane.width[0].a
                            waypoint.road_id = road.id
                            waypoint.lane_id = lane.id
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            for n_dist in range(n, -1, -1):
                                ...
                                waypoint.transform.location = self.get_point_in_line(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0,
                                    (s - waypoint_geometry_origin_lane_center.s) + (n_dist * distance),
                                    road.planView[i].hdg)
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (road.planView[i].hdg) * (180 / math.pi) + 90
                            ...
                            for n_dist in range(n, -1, -1):
                                ...
                                waypoint.transform.location = self.get_point_in_arc(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0,
                                    (s - waypoint_geometry_origin_lane_center.s) + (n_dist * distance), new_curvature,
                                    road.planView[i].hdg)
                                waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                                waypoint.s)
                                waypoint.transform.rotation.yaw = (road.planView[i].hdg + (
                ...
                for i in range(0, geometry_index + 1):
                    ...
                    if i == geometry_index:
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            waypoint.transform.location = self.get_point_in_line(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s - road.planView[i].s,
                                road.planView[i].hdg)
                            waypoint.transform.location.z = self.calculate_elevationProfile(road.elevationProfile,
                                                                                            waypoint.s)
                            waypoint.transform.rotation.yaw = (road.planView[i].hdg) * (180 / math.pi) + 90
                ...
                for i in range(geometry_index_s_0, geometry_index_s, -1):
                    ...
                    if i == geometry_index_s_0:
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            waypoint.transform.location = self.get_point_in_line(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s_0, road.planView[i].hdg)
                            ...
                            waypoint.transform.location = self.get_point_in_arc(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s_0, new_curvature,
                                road.planView[i].hdg)
                ...
                for i in range(geometry_index_s_0, geometry_index_s):
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            for n_dist in range(0, n):
                                ...
                                waypoint.transform.location = self.get_point_in_line(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0,
                                    s_0 + (n_dist * distance), road.planView[i].hdg)
                                # waypoint.transform.location.z = calculate_elevationProfile(road.elevationProfile, n_dist*distance)
                                waypoint.road_id = road.id
                                waypoint.lane_id = lane.id
                                waypoint.s = s_0 + (n_dist * distance)
                            ...
                            for n_dist in range(0, n):
                                ...
                                waypoint.transform.location = self.get_point_in_arc(
                                    waypoint_geometry_origin_lane_center.transform.location.x,
                                    waypoint_geometry_origin_lane_center.transform.location.y, 0,
                                    s_0 + (n_dist * distance), new_curvature, road.planView[i].hdg)
                                # waypoint.transform.location.z = calculate_elevationProfile(road.elevationProfile, n_dist*distance)
                                waypoint.road_id = road.id
                                waypoint.lane_id = lane.id
                                waypoint.s = s_0 + n_dist * distance
                    ...
                    # Case of last geometry
                        ...
                        if (road.planView[i].type == "line"):
                            ...
                            waypoint.transform.location = self.get_point_in_line(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s, road.planView[i].hdg)
                            # waypoint.transform.location.z = calculate_elevationProfile(road.elevationProfile, road.planView[i].length)
                            waypoint.road_id = road.id
                            waypoint.lane_id = lane.id
                            ...
                            waypoint.transform.location = self.get_point_in_arc(
                                waypoint_geometry_origin_lane_center.transform.location.x,
                                waypoint_geometry_origin_lane_center.transform.location.y, 0, s, new_curvature,
                                road.planView[i].hdg)
                            # waypoint.transform.location.z = calculate_elevationProfile(road.elevationProfile, new_length)
                            waypoint.road_id = road.id
                            waypoint.lane_id = lane.id
