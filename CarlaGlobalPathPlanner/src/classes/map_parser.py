...
class MapParser:
    ...
    def _get_param(self, param, line_str):
        ...
        """
        value = ""
        pos_ini = line_str.find(param) + len(
            param) + 1  # Position of the first value param into the sring (+1 value is because of " in xodr format)
        if (pos_ini != -1):
    ...
    def _get_param_geo(self, param, line_str):
        ...
        value = ""
        pos_ini = line_str.find(param) + len(
            param)  # Position of the first value param into the sring (+1 value is because of " in xodr format)
        if (pos_ini != -1):
            for i in range(pos_ini, len(line_str)):
                if line_str[i] == ' ':
                    break
    ...
    def _get_roads(self, xodr_map):
        ...
        for i in range(0, len(xodr_map)):
            if (in_road == 0):
                ...
                if (
                        in_roadLink == 0 and in_type == 0 and in_planView == 0 and in_elevationProfile == 0 and in_lanes == 0 and in_objects == 0 and in_signals == 0):
